            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @if(auth()->user()->role->name === 'manager')
                        <span class="text-blue-500 text-xl font-bold">Received Applications</span>
                        <div class="mt-5">
                            @foreach($applications as $application)
                            <div class="rounded-xl mt-3 border p-5 shadow-md w-9/12 bg-white">
                               ...
                               <div class="flex justify-between">
                                   ...
                                   <div>
                                        <div class="w-28 border m-6 p-6 rounded hover:bg-gray-50 transition cursor-pointer flex flex-col items-center">
                                ...
                                @if($application->answer()->exists())
                                    <div>
                                        <hr class="border">
                                        <h3 class="text-xs font-bold mt-2 text-indigo-500">Answer:</h3>
                                        <p>{{ $application->answer->body }}</p>
                                    </div>
                                @else
                                    <div class="flex justify-end">
                                        <a href="{{ route('answers.create', ['application' => $application->id]) }}" type="button" class="border border-green-500 bg-green-500 text-white text-sm rounded-md px-3 py-1 m-3 transition duration-500 ease select-none hover:bg-green-600 focus:outline-none focus:shadow-outline">
                                            Answer
                                        </a>
                                    </div>
                                @endif
